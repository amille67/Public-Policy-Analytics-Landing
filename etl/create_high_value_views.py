...
def _rollup_county(
    lowest: Any, view_id: int, fips_list: list[str] | None = None
) -> Any:
    ...
    if "geometry" not in lowest.columns or lowest.geometry.is_empty.all():
        ...
        if "county_fips" in county.columns:
            numeric = [
                c for c in county.columns if pd.api.types.is_numeric_dtype(county[c])
            ]
        ...
        if "GEOID" in lowest.columns:
            ...
            numeric_cols = [
                c for c in lowest.columns if pd.api.types.is_numeric_dtype(lowest[c])
            ]
...
def _save_outputs(lowest: Any, county: Any, view_id: int, out_dir: Path) -> None:
    ...
    if (
        hasattr(lowest, "geometry")
        and "geometry" in lowest.columns
        and not lowest.geometry.is_empty.all()
    ):
    ...
    if (
        hasattr(county, "geometry")
        and "geometry" in county.columns
        and not county.geometry.is_empty.all()
    ):
...
def run_selected(
    ...
    *,
    ...
    requested = (
        None
        if views == "all"
        else {int(x.strip()) for x in views.split(",") if x.strip()}
    )
    executed: list[int] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build Nashville high-value pre-computed views"
    )
    parser.add_argument(
        "--views", default="all", help="all or comma-separated IDs (e.g., 1,3,7)"
    )
