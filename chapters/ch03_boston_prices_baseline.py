...
def build_pipeline(cfg: Any, settings: Any, output_root: Path | None = None) -> None:
    ...
    out_dir = output_root / "ch03" if output_root else Path("outputs/ch03")
    ...
    houses_with_nhood = sjoin(houses_gdf, nhoods_gdf[[nhood_id_col or "geometry", "geometry"]] if nhood_id_col else nhoods_gdf, how="left", predicate="within")
    ...
    elif nhood_id_col in houses_with_nhood.columns:
    ...
    houses_xy = np.column_stack([houses_with_nhood.geometry.x, houses_with_nhood.geometry.y])
    ...
    exclude = {target_col, lon_col, lat_col, "geometry", "nhood_id", crime_lon, crime_lat}
    numeric_cols = [
        c for c in houses_with_nhood.select_dtypes(include="number").columns
        if c not in exclude and "Unnamed" not in c
    ]
    feature_cols = numeric_cols + ["crime_knn_mean_dist_m"]
    ...
    if len(feat_df) < 10:
        ...
        feat_df_save = houses_with_nhood[[target_col, "crime_knn_mean_dist_m", "nhood_id"]].copy()
        write_parquet(feat_df_save, out_dir / "features.parquet")
        write_json({"rmse": None, "mae": None, "r2": None, "by_neighborhood": {}}, out_dir / "model_metrics.json")
        return

    X = feat_df[feature_cols].values
    y = feat_df[target_col].values
    groups = feat_df["nhood_id"].values
    ...
    feat_out = houses_with_nhood[[target_col, "crime_knn_mean_dist_m", "nhood_id"]].copy()
    ...
    try:
        ...
        fig = pred_vs_actual(y_test, y_pred, title="Boston Home Prices: Predicted vs Actual", theme=ptheme)
        write_figure(fig, fig_dir / "pred_vs_actual.png")
    except Exception as e:
        logger.warning("Figure error: %s", e)

    logger.info("Ch03 complete. RMSE=%.2f R2=%.4f", global_metrics["rmse"], global_metrics["r2"])
